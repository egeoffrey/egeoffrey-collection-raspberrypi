...
RUN apt-get update && apt-get install -y flac alsa-utils pocketsphinx sox pulseaudio mpg123 libttspico-utils bluetooth bluez bluez-tools pulseaudio-module-bluetooth python3 python3-pip nginx rtl-sdr librtlsdr-dev python-opencv && apt-get clean && rm -rf /var/lib/apt/lists/*
RUN pip install --upgrade pip && pip install --upgrade setuptools==45.0.0 && pip install SpeechRecognition RPi.GPIO OPi.GPIO pyserial gTTS Adafruit-ADS1x15 imutils
